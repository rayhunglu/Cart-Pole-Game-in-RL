...
class VI_server_fast(object):
    ...
    def get_error_and_update(self):
        ...
        return max_error
@ray.remote
def VI_worker_fast(VI_server, data, start_state, end_state):
        ...
        # bellman backup
        max_v=[0]*(end_state-start_state)
        max_a=[0]*(end_state-start_state)
        for update_state in range(start_state, end_state):
            reward=float('-inf')
            for action in range(A):
                vcume=0
                pairs=env.GetSuccessors(update_state,action)#(s,p)=zip(next_states[0], probs[0])
                for successor in pairs: #∑T(s,a,s′)Vcurr(s′) 
                    vcume+=successor[1]*V[successor[0]]
                re=env.GetReward(update_state,action)+beta*vcume#R(s,a)+β∑T(s,a,s′)Vcurr(s′) 
                if re>reward:#max(R(s,a)+β∑s′∈ST(s,a,s′)Vcurr(s′) )
                    max_a[update_state-start_state]=action
                    max_v[update_state-start_state]=re
                    reward=re
        VI_server.update.remote(start_state, end_state, max_v, max_a)
def fast_value_iteration(env, beta = 0.999, epsilon = 0.01, workers_num = 4):
    S = env.GetStateSpace()
    VI_server = VI_server_fast.remote(S)
    workers_list = []
    data_id = ray.put((env, workers_num, beta, epsilon))
    start = 0
    batchsize=S//workers_num
    ...
    while error > epsilon:
        ...
        for i in range(workers_num):
            w_id = VI_worker_fast.remote(VI_server, data_id,start,start+batchsize)
...
print("fast_value_iteration time:", run_time['fast_value_iteration'])
